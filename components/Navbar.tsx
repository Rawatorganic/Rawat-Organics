  ...
  return (
    ...
    >
      <div className="flex justify-between items-center w-full px-6 md:px-10 py-5 max-w-screen-2xl mx-auto">
        ...
        <Link
          href="/"
          className={`flex justify-center items-center text-xl font-headline font-bold tracking-tighter transition-colors duration-300 ${scrolled ? 'text-primary' : 'text-white'
            }`}
        >
          <Image
            src={scrolled ? "/rawat.svg" : "/rawat-white.svg"}
            alt={BRAND.name}
            width={50}
            height={50}
          ...
          {links.map((link) => (
            <li key={link.label} role="none">
              <Link
                ...
                className={`relative transition-colors duration-300 group ${scrolled
                  ? isActive(link.href)
                    ? 'text-primary'
              ...
              >
                {link.label}
                <span
                  className={`absolute -bottom-1 left-0 h-px bg-current transition-all duration-300 ${isActive(link.href) ? 'w-full' : 'w-0 group-hover:w-full'
                    }`}
                />
              </Link>
            </li>
        ...
        <div className="flex items-center gap-4">
          <Link
            ...
            className={`hidden md:inline-flex px-6 py-2.5 rounded-full font-headline text-sm font-bold transition-all duration-300 hover:scale-105 active:scale-95 ${scrolled
              ...
              {links.map((link) => (
                <li key={link.label}>
                  <Link
                    href={link.href}
                    aria-current={isActive(link.href) ? 'page' : undefined}
                    className={`block text-on-surface font-headline font-semibold text-base py-3 border-b border-outline-variant/20 transition-colors ${isActive(link.href) ? 'text-primary' : 'hover:text-primary'
                      }`}
